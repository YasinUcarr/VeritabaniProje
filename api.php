<?php
/**
 * Düzeltilmiş API - api.php
 * Otopark Yönetim Sistemi
 *
 * Not: config.php dosyan zamandaki Database sınıfını içeriyor olmalı.
 */
...
// CORS (geliştirme için geniş izin, production'da daralt)
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type, X-Requested-With');
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    exit; // Preflight için hızlı çıkış
}
...
/**
 * Yardımcı: Stored procedure çağır ve ilk result set'i dön.
 * $params her zaman string/number olarak verilebilir.
 * Bu fonksiyon ayrıca MySQL'den kalan ek result set'leri temizler (next_result).
 *
 * Dönen format:
 * - multi-row result için array of assoc rows
 * - tek satır, tek sütun vs. durumlarında yine array of assoc rows (boşsa [])
 */
function callProcedureAndFetchAll($conn, $procedureName, $params = []) {
    // Oluştur param stringini: tüm paramları quote ile geçir (güvenlik: escape)
    $escaped = [];
    foreach ($params as $p) {
        // NULL desteği
        if ($p === null) {
        ...
        // sayısal ise numerik bırak, ama güvenlik için yine kontrol edelim
        if (is_int($p) || is_float($p)) {
            $escaped[] = $p;
        } else {
            // string: escape ve tırnakla sar
    ...
    // Temizle: varsa diğer result set'leri atla (important)
...
/**
 * JSON response helper (aynı config.php ile uyumlu)
 */
...
try {
    switch ($action) {
        ...
        case 'musteri_ekle':
            ...
            // Procedure geri dönüşü genelde tek satır
    ...
    break;
        ...
        case 'aktif_araclar':
            ...
            // Eğer kolon adlarını kullanıcı arayüzünde farklı bekliyorsa, adapter ekle
            // Örneğin v.Plaka yerine Plaka gibi - burada prosedürün döndürdüğü ile bıraktım.
